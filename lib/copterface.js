...
require(path.join(__dirname,'../node_modules/jsfeat/cascades/frontalface'));
require(path.join(__dirname,'../node_modules/jsfeat/cascades/bbf_face'));
require(path.join(__dirname,'../node_modules/jsfeat/cascades/bbf_face'));
            ...
            //pica.resizeBuffer({src:imageData.data, width:imageData.width, height:imageData.height,toWidth:w,toHeight:h},function(err,imageData){
	            ...
	            */
	            ...
	            if( opts.outputImage ){
		            ...
		            rects.forEach((rect)=>{
			            outputImage.fillRect(Math.round(rect.x),Math.round(rect.y),Math.round(rect.width),Math.round(rect.height), outputImage.colors.green(20));
			            outputImage.drawText(Math.round(rect.x),Math.round(rect.y),"c="+rect.confidence, outputImage.colors.black(100))
		            });
